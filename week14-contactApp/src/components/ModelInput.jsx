...
const ModelInput = ({ formik , error , fieldHasError , inputType , labelName , labelIcon }) => {
    return (
        <>
            <div className = {`${fieldHasError ? 'border-red-400 bg-red-300/5' : 'border-white/50 bg-white/20'} border-white/50 bg-white/20 border shadow-[0_5px_10px_rgba(0,0,0,0.05)] rounded-lg px-3 gap-3 flex justify-start items-center`}>
                ...
                <input type = { inputType } name = {labelName} id = {labelName} placeholder = {labelName} value = {formik.values[labelName]} onBlur = {formik.handleBlur} onChange = {formik.handleChange}  className = 'w-full py-3 text-cyan-700 placeholder:text-gray-500'/>
